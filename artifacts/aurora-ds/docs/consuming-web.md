# Consuming Aurora Design System in web apps

Read `artifacts/aurora-ds/docs/AGENTS.md` first. This guide covers
...
`artifacts/aurora-ds/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/aurora-ds/styles.css";
```
...
`node_modules/@workspace/aurora-ds/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/aurora-ds/components/ui/button";
import { cn } from "@workspace/aurora-ds/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/aurora-ds/hooks/use-toast";
```
...
`@workspace/aurora-ds/components/ui/button`. Run the app's typecheck
