# Migrating Expo UI to Aurora Design System

Read `artifacts/aurora-ds/docs/AGENTS.md` and
`artifacts/aurora-ds/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/aurora-ds/lib/native-theme` and
  `@workspace/aurora-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/aurora-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/aurora-ds` and retains only
