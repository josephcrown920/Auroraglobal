...
**Known overstatement:** env `ProviderAdapter.tasks` currently advertises every provider as capable of every task, so `providerStatus()` reports protocol availability, not true deployed-model capability. A follow-up exists to make capabilities per-backend configurable.
