...
- **GitHub raw branch names are case-sensitive** and this repo carries several
  near-identical branches (`Main`, `main`, `master`, …). A self-bootstrap fetch
  from `raw.githubusercontent.com` must try candidate branches (or take an
  override) rather than hardcode one. Private repos can't fetch over raw URLs at
  all — fall back to manually uploaded files.
