...
**How to apply:** When verifying an identity-locked generation actually worked (not just "did it return 200"), check which provider actually won — via server logs / `provider_logs` / the result's model metadata — rather than trusting the orchestrator's return value alone. See follow-up task proposal (task #249, "Warn admins when the multi-angle reshoot silently drops face-matching") for the durable fix: surface identity-capable vs. identity-blind provider status to the UI/user instead of silently succeeding.
