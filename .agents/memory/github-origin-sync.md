...
**origin/Main is a STALE, DIVERGED lineage — never `git pull` it.** The workspace and GitHub origin/Main forked long ago (seen as a large symmetric ~N↓/N↑ in the Git pane). origin/Main is *behind* the workspace: `git diff --stat HEAD origin/Main` shows it would DELETE hundreds of files / tens of thousands of lines of current work. The workspace is the source of truth — reconcile only by making GitHub match the workspace (force-push), never by pulling. The Git pane's "Pull"/"unpulled changes must be merged" nudge is misleading here; pulling starts a destructive conflicted merge.
**Auth caveat:** GitHub can show "UNAUTHENTICATED — Failed to authenticate with the remote" when no GitHub connection exists (`listConnections('github')` empty) and `GITHUB_TOKEN` secret is unset. The 10-min `scripts/github-autopush.sh` needs `GITHUB_TOKEN`; with it unset, nothing is auto-pushing and origin just stays stale. Any sync/push is blocked until the user reconnects GitHub or sets the token — the agent cannot authenticate on their behalf.
