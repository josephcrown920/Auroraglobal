...
describe("video production brain", () => {
  ...
  it("keeps a local shot edit local and follows dependencies", () => {
    ...
    brain.nodes.push(
      ...
      { id: "shot-7", type: "shot", sceneId: "scene-3", status: "approved", version: 1, dependsOn: ["scene-3"], sourceAssetIds: [], payload: {} },
      { id: "rough-cut", type: "rough-cut", status: "approved", version: 1, dependsOn: ["shot-7"], sourceAssetIds: [], payload: {} },
    ...
    brain.timeline.push({ id: "video", kind: "video", items: [
      { id: "i7", nodeId: "shot-7", start: 0, end: 5, volume: 1, opacity: 1, speed: 1, effects: [] },
      { id: "i9", nodeId: "shot-9", start: 5, end: 10, volume: 1, opacity: 1, speed: 1, effects: [] },
    ] });
    const impact = planChange(brain, "make shot 7 darker");
    expect(impact.affectedNodeIds).toContain("shot-7");
    expect(impact.staleNodeIds).toContain("shot-7");
