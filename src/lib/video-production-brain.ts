...
/**
 * Canonical Production Operating System contract shared by every Aurora video
 * agent. Agents may specialize, but they all reason over the same production
 * state, dependency graph, edit representation, and lifecycle.
 */
...
export const ProductionBrainSchema = z.object({
  version: z.literal(1),
  projectId: z.string(),
  phase: z.enum(PRODUCTION_PHASES),
  intent: z.object({
    objective: z.string(),
    audience: z.string(),
  ...
  creative: z.object({
    ...
    characterBible: z.array(z.string()).default([]),
    worldBible: z.array(z.string()).default([]),
    styleBible: z.array(z.string()).default([]),
    assumptions: z.array(z.string()).default([]),
  }),
  references: z.array(AssetRefSchema).default([]),
  nodes: z.array(ProductionNodeSchema).default([]),
  timeline: z.array(TimelineTrackSchema).default([]),
  audio: z.object({
    voiceId: z.string().optional(),
    musicDirection: z.string().default(""),
    sfxDirection: z.string().default(""),
    mixNotes: z.array(z.string()).default([]),
  }),
  delivery: z.object({
    formats: z.array(z.string()).default([]),
    resolutions: z.array(z.string()).default([]),
    captions: z.boolean().default(false),
    cleanVersion: z.boolean().default(false),
...
/** Dependency-aware impact analysis for natural-language project edits. */
export function planChange(brain: ProductionBrain, instruction: string): ChangeImpact {
  const text = instruction.toLowerCase();
  const direct: string[] = [];
  const addBy = (predicate: (n: ProductionNode) => boolean) => {
    for (const n of brain.nodes) if (predicate(n)) direct.push(n.id);
  };

  const sceneMatch = text.match(/scene\s*(\d+)/i);
  if (sceneMatch) {
    const sceneId = `scene-${sceneMatch[1]}`;
    addBy((n) => n.id === sceneId || n.sceneId === sceneId);
  }
  ...
  if (characterChange) addBy((n) => n.type === "asset" && ["identity", "wardrobe"].some((r) => n.payload.role === r) || n.type === "shot");
  ...
  if (editChange) addBy((n) => n.type === "track" || n.type === "transition" || n.type === "caption" || n.type === "effect");
  ...
  while (changed) {
    ...
    for (const n of brain.nodes) {
      if (n.dependsOn.some((id) => stale.has(id)) || n.sourceAssetIds.some((id) => stale.has(id))) {
        if (!stale.has(n.id)) { stale.add(n.id); changed = true; }
      }
    }
  }

  const affectedTrackIds = brain.timeline
    .filter((t) => t.items.some((i) => stale.has(i.nodeId) || affected.includes(i.nodeId)))
    .map((t) => t.id);

  const requiresReplan = /entire|whole|story|script|ending|duration|platform|audience|objective/.test(text) && affected.length === 0;
  if (requiresReplan) {
    for (const n of brain.nodes) stale.add(n.id);
  }
  ...
  return {
    ...
    affectedTrackIds,
    requiresReplan,
    reason: requiresReplan ? "Creative direction changed at project level." : affected.length ? "Dependency-aware local revision." : "No explicit production object matched; agent should inspect project context before acting.",
  };
}

export function buildInitialBrain(input: {
  projectId: string;
  brief: string;
  objective?: string;
  audience?: string;
  ...
  return ProductionBrainSchema.parse({
    version: 1,
    projectId: input.projectId,
    phase: "brief",
    intent: {
      objective: input.objective ?? input.brief,
      audience: input.audience ?? "Infer from the brief",
      emotionalGoal: input.emotionalGoal ?? "Infer from the brief",
      durationSeconds: input.durationSeconds,
      aspectRatios: input.aspectRatios ?? [],
    },
    creative: { brief: input.brief },
    references: [],
    nodes: [{ id: "brief", type: "brief", payload: { text: input.brief } }],
    timeline: [],
    audio: { musicDirection: "", sfxDirection: "", mixNotes: [] },
    delivery: { formats: input.aspectRatios ?? [], resolutions: [], captions: false, cleanVersion: false, variants: [] },
    history: [],
  });
}
...
export function recordInstruction(brain: ProductionBrain, instruction: string, impact: ChangeImpact, actor = "user"): ProductionBrain {
  return ProductionBrainSchema.parse({
    ...brain,
    phase: impact.requiresReplan ? "intent" : "repair",
    nodes: brain.nodes.map((n) => impact.staleNodeIds.includes(n.id) ? { ...n, status: "stale" } : n),
    history: [...brain.history, {
      id: `change-${Date.now()}-${brain.history.length + 1}`,
      at: new Date().toISOString(),
      actor,
      instruction,
      affectedNodeIds: impact.staleNodeIds,
