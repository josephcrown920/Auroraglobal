...
function secret(): string {
  return process.env.SUPABASE_SERVICE_ROLE_KEY ?? "watermark-dev-fallback";
}

/**
 * Sign a short-lived HMAC token that authorises serving the watermarked image
 * for a specific (userId, generationId) pair.  Used by listGallery (server fn)
 * to generate the URL, and by the watermark-image handler to verify it.
 */
