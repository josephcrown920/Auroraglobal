...
function parseCost(headers: Headers): { cost: number | null; source: string | null } {
  // Segmind's billing headers have changed names across API revisions. Prefer
  // x-cost as the caller-facing source of truth, then retain compatibility with
  // x-credit-cost. Never invent a cost when the provider did not return one.
  const candidates = ["x-cost", "x-credit-cost"];
...
export async function segmindSeedanceMini(
  ...
  if (!response.ok) {
    ...
    throw new Error(`Segmind Seedance 2 Mini ${response.status}: ${body}`);
  }

  const { cost, source } = parseCost(response.headers);
  const requestId = response.headers.get("x-request-id") ?? response.headers.get("x-requestid");

  // This is intentionally the only billing log emitted by this client. The
  // provider header is the authoritative observed spend for the request.
