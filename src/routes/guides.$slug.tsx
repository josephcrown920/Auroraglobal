...
/** Small upload tile for a reference slot (mirrors Colors Studio's MiniUpload:
 * own studio bucket + 1h signed URL, so ownership guards accept it). */
function SlotUpload({
  userId,
  label,
  description,
...
function GuideRunner() {
  ...
  /** Placeholder values per step: values[stepId][KEY] = user text. */
  const [values, setValues] = useState<Record<string, Record<string, string>>>({});
  /** Uploaded reference URLs per step: refs[stepId][slotKey] = signed URL. */
  const [refs, setRefs] = useState<Record<string, Record<string, string | null>>>({});
  /** Media produced by each generative step. */
  const [results, setResults] = useState<Record<string, StepResult>>({});
  /** Video steps: generation id of the succeeded 480p preview (full-quality ticket). */
  const [previewTickets, setPreviewTickets] = useState<Record<string, string>>({});
  /** Whether to chain the previous step's image (default on). */
  const [chainPrev, setChainPrev] = useState<Record<string, boolean>>({});
  /** Selected variant per step (-1 = base prompt). */
  ...
  /** Latest image produced by any step BEFORE the given index (chaining). */
  ...
  const videoMut = useMutation({
    mutationFn: (vars: { confirmPreviewId?: string }) => {
      ...
      return genVideoFn({
        data: {
          ...
          // Filled templates can exceed the server's 2500-char video prompt cap.
    ...
    onError: (err: unknown) => {
      // An expired/invalid preview ticket is rejected server-side before any
      // charge — drop the stale ticket so the button offers a fresh preview.
        ...
        <div className="rounded-2xl border border-border bg-card/40 p-5">
                ...
                {[{ label: "Base", prompt: step.promptTemplate }, ...step.variants].map((v, i) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setVariantIdx((prev) => ({ ...prev, [step.id]: idx }))
                      }
                      className={cn(
            ...
            <div className="mt-4">
              {previousImage ? (
                <button
                  type="button"
                  onClick={() =>
                    setChainPrev((prev) => ({ ...prev, [step.id]: !usePrev }))
                  }
                  className={cn(
                ...
                >
                  ...
                  <div className="flex-1 text-xs">
                    ...
                    <div className="text-muted-foreground">
                      {usePrev ? "Attached as a reference" : "Not attached"}
                    </div>
                  </div>
                  <div
                    className={cn(
          ...
          <div className="mt-5 flex flex-wrap items-center gap-2">
            {step.kind === "image" && (
              <Button
                disabled={
                  isPending || !filledPrompt.trim() || missingRequired.length > 0
                }
                onClick={() => {
