...
export const Route = createFileRoute("/api/video-agent/stream")({
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        if (!(await authUserId(request))) return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } });
        ...
        try {
          ...
          const upstream = await streamModelArkText({ messages: input.messages as ModelArkStreamMessage[], model: input.model, temperature: input.temperature, maxTokens: input.maxTokens, signal: request.signal });
          return wrapModelArkSSE(upstream, request.signal);
        } catch (error) {
          return new Response(JSON.stringify({ error: error instanceof Error ? error.message : String(error) }), { status: 500, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } });
        }
      },
    },
  },
