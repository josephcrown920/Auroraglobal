...
const ENHANCE_RATE_WINDOW_MS = 60_000;
const ENHANCE_RATE_MAX_PER_WINDOW = 8;
...
const EnhanceSchema = z.object({
  ...
  // Accepted for wire-compat with older standalone SPA builds; routing is
  // handled by the AI router's category system now, so the value is ignored.
  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        try {
          assertRateLimit(`video-agent-enhance:${userId}`, ENHANCE_RATE_MAX_PER_WINDOW, ENHANCE_RATE_WINDOW_MS);
        } catch (e) {
        ...
        try {
          ...
          const { routedGenerate } = await import("@/lib/ai-router");
          ...
          return new Response(JSON.stringify({ script, provider, model }), { headers: CORS });
        } catch (e) {
